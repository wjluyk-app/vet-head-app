...
import { calculateFridaySkins } from "@/lib/skins";

export const FRIDAY_FIELD_POT = 450;
export const FRIDAY_SKINS_POT = 200;

export interface FridayBoardTeam {
...
export interface FridayBoardSkin { hole:number; players:string; netScore:number; teamPayout:number; sourceKey:string; }
export interface FridayTournamentBoard {
  generatedAt:string; matchPlay:FridayLiveResults; teams:FridayBoardTeam[]; skins:FridayBoardSkin[];
  fieldComplete:boolean; fieldDistributed:number; skinsDistributed:number; moneyDistributed:number; moneyAvailable:number;
}
...
export function calculateFridayTournamentBoard(matches:LiveFridayMatch[]):FridayTournamentBoard {
  ...
  const teams:FridayBoardTeam[]=basics.map(r=>({scorecardId:r.card.id,sourceKey:r.card.sourceKey,matchNumber:r.matchNumber,captainTeam:r.captainTeam,captainLabel:r.captainLabel,players:r.players,scores:r.scores,front:r.front,back:r.back,total:r.total,frontRank:fr.get(r.card.id)??null,backRank:br.get(r.card.id)??null,totalRank:tr.get(r.card.id)??null,matchPoints:r.matchPoints,fieldPayout:(fp.get(r.card.id)??0)+(bp.get(r.card.id)??0)+(tp.get(r.card.id)??0),lastUpdatedAt:r.lastUpdatedAt}));
  let skins:FridayBoardSkin[]=[];
  if(fieldComplete){
    const pars=[4,3,4,4,3,5,4,4,5,4,4,3,4,5,4,4,3,5];
    const winners=calculateFridaySkins(teams.map(t=>({team:t.sourceKey,scores:t.scores as number[],pars})));
    const share=winners.length?FRIDAY_SKINS_POT/winners.length:0;
    const byKey=new Map(teams.map(t=>[t.sourceKey,t]));
    skins=winners.map(w=>({hole:w.hole,players:byKey.get(w.team)!.players,netScore:w.netScore,teamPayout:share,sourceKey:w.team}));
  }
  const fieldDistributed=teams.reduce((s,t)=>s+t.fieldPayout,0);
  const skinsDistributed=skins.reduce((s,t)=>s+t.teamPayout,0);
  return {generatedAt:new Date().toISOString(),matchPlay,teams,skins,fieldComplete,fieldDistributed,skinsDistributed,moneyDistributed:fieldDistributed+skinsDistributed,moneyAvailable:FRIDAY_FIELD_POT+FRIDAY_SKINS_POT};
}
