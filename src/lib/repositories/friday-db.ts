...
type PairingRow = {
  ...
  session: { course: string; format: string; status: string } | null;
  scorecard: Array<{
    id: string;
    source_key: string;
    team_id: string | null;
...
export async function getFridayMatchesFromDatabase(
  ...
  return ((data ?? []) as unknown as PairingRow[]).map((pairing) => {
    ...
    if (!luke || !sam) throw new Error(`Missing scorecards for Friday match ${pairing.match_number}`);
    ...
    return {
      ...
      format: pairing.session?.format ?? "1 Best Ball of 2",
