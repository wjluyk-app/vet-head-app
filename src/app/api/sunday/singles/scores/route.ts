...
const scoreInput = z.object({
  ...
  holeNumber: z.number().int().min(10).max(18),
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return Response.json(
      {
        ok: false,
        error: "Invalid Sunday Singles score request",
  ...
  const { data, error } = await supabase.rpc(
    "save_player_hole_score",
    {
      ...
      p_reason:
        parsed.data.reason ?? "Sunday Singles NET score entry",
    },
  );
  ...
  if (!saved) {
    return Response.json(
      { ok: false, error: "No score returned" },
      { status: 500 },
    );
  }
