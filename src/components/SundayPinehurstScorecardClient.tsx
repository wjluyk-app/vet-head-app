...
export default function SundayPinehurstScorecardClient({ match }: { match: LiveFridayMatch }) {
  ...
  useEffect(() => {
    ...
    const onOnline = async () => {
      ...
      if (!scoringOpen) {
        setMessage("Sunday Pinehurst scoring is not open.");
        return;
      }
    ...
    <section className="card scoreEntryCard">
      ...
      {!scoringOpen && (
        <div className="errorNotice">
          Sunday Pinehurst scoring is currently closed. Open the Sunday
          Pinehurst session from the Administrator Dashboard before entering scores.
        </div>
      )}
